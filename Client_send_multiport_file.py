...
def envia_msg_hsm(conexoes, mensagens, repeticoes, intervalo):
    total_conexoes = len(conexoes)
    total_envios = len(mensagens) * repeticoes
    print(f"\n➡️ Enviando {total_envios} mensagens (arquivo x repetições) balanceadas entre {total_conexoes} conexões...\n")

    contador = 1
    for rep in range(repeticoes):
        for msg in mensagens:
            indice = (contador - 1) % total_conexoes  # Round-robin
            s = conexoes[indice]
            try:
                ip_origem, porta_origem = s.getsockname()
                ip_destino, porta_destino = s.getpeername()

                print(f"🔹 [Conn {indice + 1}] Transação {contador}")
                print(f"   Origem: {ip_origem}:{porta_origem} → Destino: {ip_destino}:{porta_destino}")
                print(f"   Mensagem enviada: {msg}")

                t1 = datetime.datetime.now()
                enviar_mensagem(s, msg)
...
nome_arquivo = solicitar_input("Digite o nome do arquivo de mensagens (ex: mensagens.txt): ", str, "mensagens.txt")
repeticoes_arquivo = solicitar_input("Digite a quantidade de repetições do arquivo (padrão 1): ", int, 1)
intervalo = solicitar_input("Digite o intervalo entre mensagens (em segundos, ex: 1 ou 0.2): ", float, 1.0)

# Leitura das mensagens do arquivo
try:
    with open(nome_arquivo, 'r', encoding='utf-8') as f:
        mensagens = [linha.strip() for linha in f if linha.strip()]
    if not mensagens:
        print(f"Arquivo '{nome_arquivo}' está vazio ou inválido. Encerrando.")
        exit(1)
except Exception as e:
    print(f"Erro ao ler arquivo de mensagens: {e}")
    exit(1)
...
try:
    envia_msg_hsm(conexoes, mensagens, repeticoes_arquivo, intervalo)
finally:
